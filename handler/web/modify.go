...
func (a *modifyWeb) AddTask(w http.ResponseWriter, r *http.Request) {
	...
	// TODO: answer here
	tmpl := template.Must(template.ParseFS(a.embed,
		path.Join("views", "main", "add-task.html"),
		path.Join("views", "general", "header.html"),
	))
	err := tmpl.Execute(w, map[string]interface{}{"CatID": catId})
	if err != nil {
...
func (a *modifyWeb) AddCategory(w http.ResponseWriter, r *http.Request) {
	// TODO: answer here
	var tmpl = template.Must(template.ParseFS(a.embed,
		path.Join("views", "main", "add-category.html"),
		path.Join("views", "general", "header.html"),
	))
...
func (a *modifyWeb) UpdateTask(w http.ResponseWriter, r *http.Request) {
	...
	// TODO: answer here
	var tmpl = template.Must(template.ParseFS(a.embed, path.Join("views", "main", "update-task.html"), path.Join("views", "general", "header.html")))
